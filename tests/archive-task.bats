...
@test "concurrent invocations on different tasks in the same month preserve both bodies (lock fix)" {
  # Without the per-month mkdir lock, two simultaneous invocations both pass
  # the `[ ! -f $ARCHIVE_FILE ]` check, both create their own tmp+mv, and
  # the second mv silently clobbers the first task's body. With the lock,
  # both bodies must survive in the archive.
  cat > "$TMP_PROJECT/.redeye/tasks.md" <<'EOF'
## CEO Requests

### T001: Concurrent task one
- **Type:** feature
- **Status:** done
- **Merged:** 2026-04-01 (iter 1)
- **Description:** Body of task one — must survive.

### T002: Concurrent task two
- **Type:** feature
- **Status:** done
- **Merged:** 2026-04-01 (iter 2)
- **Description:** Body of task two — must survive.
EOF

  # Run both archive invocations concurrently with no archive file present.
  "$REDEYE_ROOT/scripts/archive-task.sh" "$TMP_PROJECT" T001 &
  pid1=$!
  "$REDEYE_ROOT/scripts/archive-task.sh" "$TMP_PROJECT" T002 &
  pid2=$!
  wait $pid1
  wait $pid2
  ...
  archive="$TMP_PROJECT/docs/tasks-archive/2026-04.md"
  [ -f "$archive" ]
  grep -q "Body of task one" "$archive" || { echo "T001 body missing"; cat "$archive"; return 1; }
  grep -q "Body of task two" "$archive" || { echo "T002 body missing"; cat "$archive"; return 1; }
  grep -q "^### T001:" "$archive"
  grep -q "^### T002:" "$archive"
}
