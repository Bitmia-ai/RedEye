...
# Gitignore every file inside `.redeye/`. The control files are runtime state
# owned by RedEye + CEO — they churn every iteration (status, state, tasks,
# changelog, etc.) and would explode git history if tracked. Multi-instance
# coordination across machines via tracked `.redeye/` is out of scope for
# the default install; instances on the same machine share through the
# local filesystem.
GITIGNORE_ENTRIES=(
  '.env.test'
  '.redeye/state.json'
...
cd "$PROJECT_ROOT"
# Stage only the project-level surface — `.gitignore` (now lists every
# `.redeye/*` file) and the `docs/` placeholders. The control files
# themselves are intentionally unstaged: they're gitignored runtime state.
git add .gitignore
