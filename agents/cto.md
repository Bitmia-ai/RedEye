...
From STABILIZE → TRIAGE (if fixed) | STABILIZE (retry, attempts < 3) | TRIAGE+park (if max attempts) or TRIAGE (if fixed, no worktree) or STABILIZE (retry) or teardown worktree → park task and post to inbox (if max attempts reached)
